...
/**
 * Author  : Bytefish (http://bytefish.de/)
 * Modifier: Reza Adhitya Saputra (reza.adhitya.saputra@gmail.com)
 * Version : 2014
 *
 */
...
// mask should be 8UC1
template <typename _Tp>
void lbp::histogramMask_(const Mat& src, Mat& hist, Mat& mask, int numPatterns)
{
	hist = Mat::zeros(1, numPatterns, CV_32SC1);
	for(int i = 0; i < src.rows; i++) 
	{
...
void lbp::histogramMask(const Mat& src, Mat& hist, Mat& mask, int numPatterns)
{
	switch(src.type()) 
	{
		case CV_8SC1: histogramMask_<char>(src, hist, mask,numPatterns); break;
		case CV_8UC1: histogramMask_<unsigned char>(src, hist, mask,numPatterns); break;
		case CV_16SC1: histogramMask_<short>(src, hist, mask,numPatterns); break;
		case CV_16UC1: histogramMask_<unsigned short>(src, hist, mask,numPatterns); break;
		case CV_32SC1: histogramMask_<int>(src, hist, mask,numPatterns); break;
	}
}
...
void lbp::spatial_histogram(const Mat& src, Mat& dst, int numPatterns, int gridx, int gridy, int overlap) {
	int width = static_cast<int>(std::floor((double)src.cols/gridx));
	int height = static_cast<int>(std::floor((double)src.rows / gridy));
